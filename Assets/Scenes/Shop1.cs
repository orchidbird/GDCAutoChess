				...
				if (a.playerunitlevel1[a.num[i] - 1] == 3)         //뽑힌 카드개수가 3개가 되면
				{
                    ...
                    for (int k = 0; k < 12; k++)
                    {
                        if (a.board[k] == a.num[i] && a.boardLevel[k] == 1)
                        {
                            ...
                            Board[k].transform.GetChild(0).GetChild(0).GetComponent<Image>().sprite = Resources.Load("창고", typeof(Sprite)) as Sprite; ;//창고가 비었을 때 다시 창고 이미지 보여주기
                            ...
                            if (b == 0)
                            {
                                ...
                                Board[k].transform.GetChild(0).GetChild(0).GetComponent<Image>().sprite = Resources.Load("카드템플릿_2", typeof(Sprite)) as Sprite;
                                b++;        // b=0일때만 작업 수행하기 때문에, 그 뒤로 발견되는 1성 카드들은 그냥 사라지기만 할 것이다.
                            }
                        }
                    }
                    ...
                    for (int k = 0; k < 12; k++)
                    {
                        if (a.board[k] == a.num[i] && a.boardLevel[k] == 2)
                        {
                            ...
                            if (b == 0)
                            {
                                ...
                                Board[k].transform.GetChild(0).GetComponent<Image>().sprite = Resources.Load(a.num[i].ToString(), typeof(Sprite)) as Sprite;
                                Board[k].transform.GetChild(0).GetChild(0).GetComponent<Image>().sprite = Resources.Load("카드템플릿_3", typeof(Sprite)) as Sprite;
                                b++;        // b=0일때만 작업 수행하기 때문에, 그 뒤로 발견되는 1성 카드들은 그냥 사라지기만 할 것이다.
                            }
                        }
                    }
